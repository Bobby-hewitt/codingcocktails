...
class CssContainer extends Component {
  ...
  render() {
    return (
        ...
        <Comment lang='css' comment="Lets start by styling the page header. The '#' grabs the ID from the HTML. This element is 'fixed' in the top left corner.  This means it doesn't move when you scroll. Try adding a border color of 'grey', a border-width of '2px' and a border-style of 'solid'"/>
        ...
        <Comment lang='css' comment="The green writing is called the 'selector'. This selector would affect all the <h1> tags inside the #headerContainer.  We've imported some fonts for you to try out.  Copy one from the comment at the top of the page and paste it into the font-family property.  Then try setting the font size to '20px' and choose a color."/>
    	...
    	<Comment lang='css' comment="The '.' refers to classes specified in the HTML.  This selector will style all the elements with the class name 'contentBlock'.  It all looks a bit cramped at the moment so try adding a margin-top of '20px'"/>
    	...
    	<Comment lang='css' comment="You can select items with their tags too.  try setting the size and color of the <h3> tags.  If we were to always write the names of colors we'd be very limited.  You can use the red green and blue values to create the exact color ou want.  black is rgb(0,0,0) and white is rgb(255,255,255). You can find the right colour at https://www.webpagefx.com/web-design/color-picker. "/>	
	   ...
	   <Comment lang='css' comment="You can turn bullet points off by setting list style type to 'none', or you can change these to numbers by setting it to 'decimal"/>
	    ...
	    <Comment lang='css' comment="Now lets sytle the footer. You know how to do this by now."/>
        ...
        <Comment lang='css' comment="Finally style the text in the footer."/>   
            <CssDeclarator changeCss={this.changeCss.bind(this)} item="footerContainer p" declarator="#footerContent" properties={this.props.footerContentCss}/>
       <Comment lang='css' comment="That's it.  You've got the basics.  Now go back and have a play. Make it look as nice as you can.  Once you're done click 'confirm order' to save it to the interwebs!"/>   
